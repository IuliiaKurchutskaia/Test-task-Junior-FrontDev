...
el: "#personal",
    ...
    <input class="text" v-model.trim="surname" name="surname" placeholder="Фамилия">


    <label class="title" for="name">Имя</label>
    <input class="text" v-model.trim="name" name="name" placeholder="Имя">
   

    <label class="title" for="SecondName">Отчество</label>
    <input class="text" v-model.trim="secondName" name="secondName" placeholder="Отчество">
    

    <label class="title" for="birthday">Дата рождения</label>
    <input class="text date" name="birthday" type="date" :value="myDate && myDate.toISOString().split('T')[0]"
                     @input="myDate = getDateClean($event.target.valueAsDate)">
                   
    <label class="title" for="phone">Номер телефона</label>
  
    <input class="text tel" type="tel" name="phone" id="phone" placeholder="(555) 555-5555" autocomplete="tel" maxlength="14"  pattern="[(][0-9]{3}[)] [0-9]{3}-[0-9]{4}" required />
   ...
   <span class="title">Группа клиентов: {{ clientGr }}</span>
    ...
    <form>
    <span class="title">Лечащий врач: {{ doctor }}</span>
    ...
    <form>
    <label class="title" for="sms">Не отправлять СМС</label>
...
new Vue({
    ...
    <label  class="title" for="index">Индекс</label>
    <input class="text"  v-model.number="index" name="index" placeholder="Индекс">
 ...
 <label  class="title" for="region">Область</label>
 ...
 <label  class="title" for="str">Улица</label>
<input class="text" v-model.trim="str" name="str" placeholder="улица">

<label  class="title" for="house">Дом</label>
<input class="text" v-model.number="house" name="house" placeholder="Дом">
...
el: "#passport",
    ...
    <div class="pages">
    <div class="page adress">
 <span  class="title" >тип документа: {{ doc }}</span>
 <select class="select" v-model="doc">
   <option  v-for="doc in docs" v-bind:value="doc.value">
   {{ doc.text }}
 </option>
 </select>

 <label  class="title" for="ser">Серия</label>
 <input class="text" v-model.number="ser" name="ser" placeholder="Серия">

 <label  class="title" for="num">Номер</label>
 <input class="text" v-model.number="ser" name="num" placeholder="Номер">

 <label  class="title" for="by">Кем выдан</label>
 <input class="text" type="by" id="by" value="Кем выдан" v-model="by">

 <label  class="title" for="dataOf">Дата выдачи</label>
 <input class="text" name="dataOf" type="date" :value="myDate && myDate.toISOString().split('T')[0]"
                  @input="myDate = getDateClean($event.target.valueAsDate)">
